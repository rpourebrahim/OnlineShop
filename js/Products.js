...
for (let i = 0; i < imgs.length; i++) {
    ...
    (i >= 9) ? n = (i + 1): n = '0' + (i + 1);
    imgs[i].addEventListener('click', function() {
        document.querySelector('#pics div').setAttribute('id', "picInfo2");
        document.querySelector('#picInfo2').innerHTML = this.getAttribute('data-desc') + "<br>$ " + this.getAttribute('data-prc');
    });
}